...
This paper reports on a production system, internally called *squad*, that was built to run a real, revenue-bearing portfolio rather than a benchmark. Over roughly a year of daily use, its design center of gravity migrated away from the agents and toward the machinery that keeps the agents honest. We make an explicit, deliberately narrow claim:
