...
O *business-agents* é um plugin para um runtime de agentes (Claude Code) e é composto por 23 agentes em três times (10 de engenharia, 8 de marketing, 5 de diretoria), cada time liderado por um coordenador, todos respondendo a um único operador humano que age como autoridade de decisão. Ele traz 35 "skills" procedurais (procedimentos executáveis com gates explícitos, não trechos de prompt) e um conjunto de ferramentas: um gate estrutural, um runner de tarefa headless, hooks de git e de ciclo de vida, um harness de avaliação e um gate de paridade de publicação. O sistema toca um portfólio vivo de aproximadamente 36 projetos, de 6 clientes.
